...
class OrderController extends Controller
{
    ...
    public function index()
    {
        $orders = Order::select(
                ...
                // 'tr_order_details.productpicture_title as image',
            )
            ->leftJoin('tr_points_histories', 'tr_points_histories.order_id', '=', 'tr_orders.id')
            // ->leftJoin('tr_order_details', 'tr_order_details.order_id', '=', 'tr_orders.id')
        ...
        foreach ($orders as $order) {
            ...
            if($orderSayur){
                $orderSayur->count = $countSayur - 1;
                $order->sayur = [$orderSayur];
            }else{
                $order->sayur = [];
            }
            
            if($orderBunga){
                $orderBunga->count = $countBunga - 1;
                $order->bunga = [$orderBunga];
            }else{
                $order->bunga = [];
            }
