    ...
    public function shop(Request $request)
    {
        ...
        foreach ($productContainers as $product) {
            ...
            if (count($kelengkapanIds)) {
                ...
                foreach ($kelengkapanIds as $isimykelengkapan){
                    $myKelengkapans = Product::withMinimalDetailWhere($wilayah->id)
                    ->where('products.id', $isimykelengkapan->kelengkapan_id)
                    ->get();
                    $arraykelengkapan[] = $myKelengkapans[0];
                ...
                //    'data'=> $arraykelengkapan[1]->product_id
                // ]);die();

                // print_r($arraykelengkapan);die();
                // var_dump($arraykelengkapan);die();
                $arrayanjg= [];
                foreach ($arraykelengkapan as  $kelengkapan) {
                    $arrayanjg[] =  $kelengkapan;
